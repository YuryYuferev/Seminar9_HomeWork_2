import telebot
from working_with_datebase import print_book, find_in_book, book, add_contact, import_base, ex_base
import log_generate as lg

bot = telebot.TeleBot('5967480382:AAHLa81klV_nbvonT20FlOXKAhjpsEubww')
chat_id = ''
surname = ''
name = ''
patronymic = ''
...
def choice(message):
    ...
    if message.text == '1':
        ...
        bot.send_message(chat_id, print_book())
...
def set_data(mess):  # Заносим в переменную телефон и записываем контакт в базу
    ...
    add_contact(surname, name, patronymic, email_address, telephone)
...
def import_base(message):
    ...
    src = '"D:\Рабочий стол\Документы\import.txt' + \
...
def export_file(message):
    if message.text == '1':
        lg.write_data(f'Запущен экспорт 1')
        ex_base(message.text)
        bot.send_document(chat_id, open(r'"D:\Рабочий стол\Документы\export.csv', 'rb'))
    elif message.text == '2':
        lg.write_data(f'Запущен экспорт2')
        ex_base(message.text)
        bot.send_document(chat_id, open(r'"D:\Рабочий стол\Документы\export2.csv', 'rb'))
    else:
